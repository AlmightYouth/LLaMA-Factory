...
class FinetuningArguments(FreezeArguments, LoraArguments, RLHFArguments, GaloreArguments, BAdamArgument):
    ...
    epsilon: float = field(
        default=5e-3,
        metadata={"help": "Maximum perturbation allowed for FGSM/PGD."}
    )
    epsilon_min: float = field(
        default=5e-4,
        metadata={"help": "Minimum perturbation allowed after decay for FGSM/PGD."}
    )
    decay_steps: int = field(
        default=2000,
        metadata={"help": "Number of steps over which epsilon decays to epsilon_min."}
    )
    pgd_step_size: float = field(
        default=2e-4,
        metadata={"help": "Step size for each PGD iteration (alpha)."}
    )
    pgd_steps: int = field(
        default=3,
        metadata={"help": "Number of PGD iterations."}
    )
    attack_method: Literal["fgsm", "pgd", "random","fgm","pgd_l2"] = field(
        default="pgd",
        metadata={"help": "Adversarial attack method to use: 'fgsm' or 'pgd'."}
    )
    perturbation_sample_ratio: float = field(
        default=0.1,
        metadata={"help": "The ratio of perturbation sample"}
    )
